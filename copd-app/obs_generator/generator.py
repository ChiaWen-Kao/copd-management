...
def generate_steps(activity, impact):
    ...
    if impact == "low":
        ...
        active_step_range = (50, 150)    # Higher step count during normal activity
    elif impact == "medium":
        workout_step_range = (200, 400)  # Medium step count during workout for medium impact
        active_step_range = (50, 100)     # Moderate step count during normal activity
    else:  # high impact
        workout_step_range = (100, 300)  # Lower step count during workout for high impact
        active_step_range = (15, 50)      # Lower step count during normal activity
...
start_time = start_time.replace(day=(start_time.day - days_interval))
...
observations_medium = generate_observations(start_time, interval_minutes=60, total_days=days_interval, impact="medium")
with open('./generated/observations_medium.json', 'w') as f:
    json.dump(observations_medium, f, indent=4)

observations_low = generate_observations(start_time, interval_minutes=60, total_days=days_interval, impact="low")
with open('./generated/observations_low.json', 'w') as f:
    json.dump(observations_low, f, indent=4)

observations_high = generate_observations(start_time, interval_minutes=60, total_days=days_interval, impact="high")
